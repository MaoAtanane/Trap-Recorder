...
import { Target, CheckCircle, XCircle, RotateCcw, Save, Clock, Zap, Settings } from "lucide-react"
...
export default function GameScoring({ userId }: GameScoringProps) {
  ...
  const updateShotScore = (shotIndex: number) => {
    ...
    const hitCount = updatedShots.filter((s) => s.firstShot === "hit" || s.secondShot === "hit").length
    ...
    localStorage.setItem(`dtl-current-game-${userId}`, JSON.stringify(updatedGame))

    // Check if game is complete
    const allShotsComplete = updatedShots.every((s) => s.score !== null)
  ...
  const completeGame = () => {
    ...
    const existingGames = JSON.parse(localStorage.getItem(`dtl-games-${userId}`) || "[]")
  ...
  return (
    <div className="space-y-4 p-4 max-w-6xl mx-auto">
      <GameHeader game={currentGame} guns={guns} chokes={chokes} ammunition={ammunition} onReset={resetGame} />
      ...
      <Dialog open={showCompletionDialog} onOpenChange={setShowCompletionDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Félicitations ! Vous avez terminé votre série DTL.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="text-center">
              <div className="text-3xl font-bold text-blue-600">{currentGame.totalScore}/75</div>
              <p className="text-slate-600">Score Final</p>
            </div>
            <div className="grid grid-cols-2 gap-4 text-center">
              <div>
                <div className="text-xl font-semibold">{currentGame.hitCount}/25</div>
                <p className="text-sm text-slate-600">Plateaux Touchés</p>
              </div>
              <div>
                <div className="text-xl font-semibold">{Math.round((currentGame.hitCount / 25) * 100)}%</div>
                <p className="text-sm text-slate-600">Taux de Réussite</p>
              </div>
            </div>
            <Button onClick={completeGame} className="w-full bg-blue-600 hover:bg-blue-700">
...
function GameSetup({
  ...
  useEffect(() => {
    ...
    if (chokes.length === 1 && !setup.overChokeId) {
      setSetup((prev) => ({ ...prev, overChokeId: chokes[0].id, underChokeId: chokes[0].id }))
    }
  ...
  }, [guns, chokes, ammunition, clubs, setup.gunId, setup.overChokeId, setup.ammunitionId, setup.club])
      ...
      <CardContent className="space-y-6">
        <div className="p-4 sm:p-6 bg-blue-50 rounded-lg border border-blue-200">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
            <div>
              <h3 className="text-base sm:text-lg font-semibold text-blue-900 mb-2">Démarrage Rapide</h3>
              <p className="text-blue-700 text-sm">Commencez immédiatement avec vos équipements par défaut.</p>
            </div>
            <Button
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Select
                    value={setup.gunId}
                    onValueChange={(value) => setSetup((prev) => ({ ...prev, gunId: value }))}
                  >
                    <SelectTrigger>
                  ...
                  {guns.length === 0 && (
                    <p className="text-sm text-slate-600">Aucun fusil disponible. Ajoutez d'abord votre équipement.</p>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {clubs.length > 0 ? (
                    <Select
                      value={setup.club}
                      onValueChange={(value) => setSetup((prev) => ({ ...prev, club: value }))}
                    >
                      <SelectTrigger>
                    ...
                    <Input
                      id="club"
                      value={setup.club}
                      onChange={(e) => setSetup((prev) => ({ ...prev, club: e.target.value }))}
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setSetup((prev) => ({ ...prev, ammunitionId: value }))}
                  >
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setSetup((prev) => ({ ...prev, overChokeId: value }))}
                  >
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setSetup((prev) => ({ ...prev, underChokeId: value }))}
                  >
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    id="weather"
                    value={setup.weather}
                    onChange={(e) => setSetup((prev) => ({ ...prev, weather: e.target.value }))}
      ...
      <CardHeader className="pb-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
          <div>
            ...
            <CardDescription className="text-sm">
              {new Date(game.date).toLocaleDateString("fr-FR")} à {game.club || "Lieu non spécifié"}
            </CardDescription>
          </div>
          <Button variant="outline" onClick={onReset} className="text-red-600 hover:text-red-700 bg-transparent h-10">
...
function ScoringGrid({ game, onUpdateShot }: { game: Game; onUpdateShot: (shotIndex: number) => void }) {
  return (
    <Card>
      <CardHeader className="pb-4">
        <CardTitle className="text-lg">Grille de Score</CardTitle>
        <CardDescription className="text-sm">Cliquez : Vide → 3pts → 2pts → 0pt → Vide</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-6">
          {[1, 2, 3, 4, 5].map((station) => (
            <div key={station} className="space-y-3">
              <h3 className="font-semibold text-base sm:text-lg">Poste {station}</h3>
  ...
  return (
    <div className="text-center">
      ...
      <div className="flex justify-center space-x-1 mt-1">
        {shot.firstShot === "hit" && <CheckCircle className="h-3 w-3 text-green-600" />}
        {shot.firstShot === "miss" && <XCircle className="h-3 w-3 text-red-600" />}
        {shot.secondShot === "hit" && <CheckCircle className="h-3 w-3 text-yellow-600" />}
        {shot.secondShot === "miss" && <XCircle className="h-3 w-3 text-red-600" />}
      </div>
    </div>
  )
}
...
function GameStats({ game }: { game: Game }) {
  const stationStats = [1, 2, 3, 4, 5].map((station) => {
    ...
    const stationScore = stationShots.reduce((sum, s) => sum + (s.score || 0), 0)
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-center space-y-2">
            <div className="text-3xl sm:text-4xl font-bold text-blue-600">{game.totalScore}/75</div>
            <div className="text-base sm:text-lg text-slate-600">{game.hitCount}/25 plateaux touchés</div>
            <div className="text-sm text-slate-500">{Math.round((game.hitCount / 25) * 100)}% de réussite</div>
          </div>
        </CardContent>
      </Card>
